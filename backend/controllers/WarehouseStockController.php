...
class WarehouseStockController {
    ...
    public static function index() {
        ...
        Pagination::run($sql, $params, $page, $limit);
    }
    ...
    public static function updateThreshold($id) {
        ...
        if (!isset($body['nguong_canh_bao'])) {
            Response::error(400, "Dữ liệu không hợp lệ: Vui lòng cung cấp 'nguong_canh_bao'");
        }
        
        $nguong_canh_bao = (int)$body['nguong_canh_bao'];
        if ($nguong_canh_bao < 0) {
            Response::error(400, "Ngưỡng cảnh báo không hợp lệ (Phải lớn hơn hoặc bằng 0)");
        }
        ...
        if (!$stmtCheck->fetch()) {
            Response::error(404, "Bản ghi tồn kho không tồn tại");
        }
